                        ...
                        <tbody>
                            @foreach ($blogs as $blog)
                                <tr>
                                    ...
                                    <td>{{ $blog->title }}</td>
                                    <td><img src="@if (!empty($blog->post_image)) {{ asset($blog->post_image) }} @endif"
                                            class="img-fluid" width="200px" height="200px"></td>
                                    <td>{{ $blog->category->name }}</td>
                                    <td>{!! Str::limit($blog->body, 15) !!}</td>
                                    <td>
                                        <div class="d-flex order-actions">
                                            <a href="{{ route('admin.posts.edit', ['post' => $blog->id]) }}" class=""><i
                                                    class='bx bxs-edit'></i></a>
                                            <a href=""
                                                onclick="event.preventDefault();document.getElementById({{ $blog->id }}).submit();"
                                                class="ms-3"><i class='bx bxs-trash'></i></a>
                                        </div>
                                        <form action="{{ route('admin.posts.destroy', ['post' => $blog->id]) }}"
                                            id="{{ $blog->id }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
